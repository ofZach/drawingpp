...
void ofApp::update(){
    ...
    for (int z = 0; z < 10; z ++){
        ...
        if (lines.size() > 0){
            ...
            float randomPct = ofRandom(0.7,1);
            
            ofPoint diff = lines[whichLine].b - lines[whichLine].a;
            float origAngle = atan2(diff.y, diff.x);
            angle = origAngle + ofRandom(-0.3, 0.3);
            //angle = origAngle + (ofRandom(0,1) > 0.5 ? -1 : 1) * (PI/2.0 + ofRandom(-0.04, 0.04));
