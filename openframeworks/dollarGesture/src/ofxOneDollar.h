...
struct ofxGesture {
	...
	void addPoint(double nX, double nY) {
		points.push_back(ofPoint(nX, nY));
	}

	void draw() {
		std::vector<ofPoint>::iterator it = points.begin();
		...
		glPointSize(4);
		std::vector<ofPoint>::iterator it_res = resampled_points.begin();
		glBegin(GL_POINTS);
	...
	void resample(int n) {
		...
		for(int i = 1; i < points.size(); ++i) {
			ofPoint curr = points[i];
			ofPoint prev = points[i-1];
			ofPoint dir = prev - curr;
			double d = dir.length();
			if( (D + d) >= I) {
				double qx = prev.x + ((I-D)/d) * (curr.x - prev.x);
				double qy = prev.y + ((I-D)/d) * (curr.y - prev.y);
				ofPoint resampled(qx, qy);
	...
	ofPoint centroid() {
		double x = 0;
		double y = 0;
		std::vector<ofPoint>::iterator it = resampled_points.begin();
	...
	double indicativeAngle() {
		ofPoint c = centroid(); // TODO: optimize
	...
	vector<ofPoint> rotateBy(vector<ofPoint> oPoints, double nRad) {
		vector<ofPoint> rotated;
		ofPoint c = centroid(); // TODO: optimize
		center = c; // TODO optimize
		double cosa = cos(nRad);
		double sina = sin(nRad);
		vector<ofPoint>::iterator it = oPoints.begin();
	...
	ofxOneDollarRect boundingBox() {
		...
		std::vector<ofPoint>::const_iterator it = resampled_points.begin();
		while(it != resampled_points.end()) {
			ofPoint v = (*it);
	...
	double distanceAtAngle(double nAngle, ofxGesture* pGesture) {
		vector<ofPoint> points_tmp = resampled_points;
		points_tmp = rotateBy(points_tmp, nAngle);
		return pathDistance(points_tmp, pGesture);
	}

	// distance between two paths.
	double pathDistance(vector<ofPoint> oPoints, ofxGesture* pGesture) {
		// sizes are not equal (?)
	...
	friend ostream& operator <<(ostream& os, const ofxGesture& rGesture) {
		...
		std::vector<ofPoint>::const_iterator it = rGesture.resampled_points.begin();
	...
	friend istream& operator >>(istream& is, ofxGesture& rGesture) {
		...
		while(is) {
			...
			if(is) {
				rGesture.resampled_points.push_back(ofPoint(x,y));
			}
		}
		return is;
	}
	...
	std::vector<ofPoint> resampled_points;
	double golden_ratio;
